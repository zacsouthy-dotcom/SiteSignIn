    ...
    .map(
      ...
      <li class="entry">
        <div>
          ...
          <div class="muted small">In at ${e.time_in}</div>
        </div>
...
app.post("/site/:slug/signin", (req, res) => {
  ...
  db.prepare(
    "INSERT INTO entries (site_id, name, company, date, time_in) VALUES (?, ?, ?, ?, ?)"
  ).run(site.id, name, company, today(), nowTime());
...
app.get("/site/:slug/export", (req, res) => {
  ...
  const header = "Date,Name,Company,Time In,Time Out\n";
  const csvRows = entries
    .map((e) =>
      [displayDate(e.date), e.name, e.company || "", e.time_in, e.time_out || ""]
        .map(csvEscape)
        .join(",")
    )
    .join("\n");
