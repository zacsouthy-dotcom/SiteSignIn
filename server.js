...
app.get("/site/:slug", (req, res) => {
  ...
  res.send(
    ...
    <div class="card">
      <h2>Sign in</h2>
      <form method="post" action="/site/${site.slug}/signin" class="stack" id="signin-form">
        <input name="name" id="f-name" placeholder="Full name" required autocomplete="name">
        <input name="company" id="f-company" placeholder="Company">
        <input name="phone" id="f-phone" placeholder="Phone number" type="tel" autocomplete="tel">
    ...
    <script>
      // Remember this person's details on this device, so they don't
      // have to retype name/company/phone every time they tap the tag.
      (function () {
        var fields = ["name", "company", "phone"];
        fields.forEach(function (f) {
          var saved = localStorage.getItem("lls_" + f);
          if (saved) document.getElementById("f-" + f).value = saved;
        });
        document.getElementById("signin-form").addEventListener("submit", function () {
          fields.forEach(function (f) {
            localStorage.setItem("lls_" + f, document.getElementById("f-" + f).value);
          });
        });
      })();
    </script>
